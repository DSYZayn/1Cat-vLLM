...
if [[ "${RUST_CHANGED}" != "true" ]]; then
  ...
  curl -fL \
    ...
    "${BASE_WHEEL_URL}" \
    -o "${baseline_wheel}"
  if unzip -p "${baseline_wheel}" vllm/vllm-rs > vllm/vllm-rs \
    && test -s vllm/vllm-rs; then
    chmod +x vllm/vllm-rs
  else
    rm -f vllm/vllm-rs
    echo "Baseline wheel has no vllm/vllm-rs; building the Rust frontend from source."
    export RUST_CHANGED=true
    export VLLM_REQUIRE_RUST_FRONTEND=true
  fi
fi
...
install_dependencies() {
  local index_url="$1"
  UV_INDEX_URL="${index_url}" uv pip install \
...
for attempt in 1 2 3; do
  if install_dependencies "${PYPI_MIRROR_URL:-https://pypi.tuna.tsinghua.edu.cn/simple}"; then
    dependency_install_succeeded=true
    break
  fi
  echo "Dependency install from the package mirror failed (${attempt}/3)." >&2
  uv cache clean >/dev/null 2>&1 || true
  sleep $((attempt * 5))
done

if [[ "${dependency_install_succeeded}" != "true" ]]; then
  echo "Package mirror install failed; retrying through PyPI." >&2
  for attempt in 1 2 3; do
    if install_dependencies "https://pypi.org/simple"; then
      dependency_install_succeeded=true
      break
    fi
    echo "Dependency install from PyPI failed (${attempt}/3)." >&2
    uv cache clean >/dev/null 2>&1 || true
    sleep $((attempt * 5))
  done
fi
...
if [[ "${dependency_install_succeeded}" != "true" ]]; then
  echo "Dependency installation failed after retries." >&2
  exit 1
fi
